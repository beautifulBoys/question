<template>
    <div>
                ...
                <div>
                    ...
                    <ul>
                        <li v-for="(a, index1) in data[qIndex].items" v-if="index1 === 0" @click="checkEvent(index1)"
                            ...
                            <span :class="[{sign1: index1 === rightIndex}, {sign2: index1 === errorIndex}, 'sign']">A</span>
                            {{a}}
                        </li>
                        <li v-for="(a, index1) in data[qIndex].items" v-if="index1 === 1" @click="checkEvent(index1)"
                            :class="[{active: activeIndex === index1}, {color1: index1 === rightIndex}, {color2: index1 === errorIndex}]">
                            <span :class="[{sign1: index1 === rightIndex}, {sign2: index1 === errorIndex}, 'sign']">B</span>
                            {{a}}
                        </li>
                        <li v-for="(a, index1) in data[qIndex].items" v-if="index1 === 2" @click="checkEvent(index1)"
                            :class="[{active: activeIndex === index1}, {color1: index1 === rightIndex}, {color2: index1 === errorIndex}]">
                            <span :class="[{sign1: index1 === rightIndex}, {sign2: index1 === errorIndex}, 'sign']">C</span>
                            {{a}}
                        </li>
                        <li v-for="(a, index1) in data[qIndex].items" v-if="index1 === 3" @click="checkEvent(index1)"
                            :class="[{active: activeIndex === index1}, {color1: index1 === rightIndex}, {color2: index1 === errorIndex}]">
                            <span :class="[{sign1: index1 === rightIndex}, {sign2: index1 === errorIndex}, 'sign']">D</span>
                            {{a}}
                        </li>
                        <li v-for="(a, index1) in data[qIndex].items" v-if="index1 === 4" @click="checkEvent(index1)"
                            :class="[{active: activeIndex === index1}, {color1: index1 === rightIndex}, {color2: index1 === errorIndex}]">
                            <span :class="[{sign1: index1 === rightIndex}, {sign2: index1 === errorIndex}, 'sign']">E</span>
                            {{a}}
                        </li>
                        <li v-for="(a, index1) in data[qIndex].items" v-if="index1 === 5" @click="checkEvent(index1)"
                            :class="[{active: activeIndex === index1}, {color1: index1 === rightIndex}, {color2: index1 === errorIndex}]">
                            <span :class="[{sign1: index1 === rightIndex}, {sign2: index1 === errorIndex}, 'sign']">F</span>
                            {{a}}
                        </li>
                        <li v-for="(a, index1) in data[qIndex].items" v-if="index1 === 6" @click="checkEvent(index1)"
                            :class="[{active: activeIndex === index1}, {color1: index1 === rightIndex}, {color2: index1 === errorIndex}]">
                            <span :class="[{sign1: index1 === rightIndex}, {sign2: index1 === errorIndex}, 'sign']">G</span>
                            {{a}}
                        </li>
                        <li v-for="(a, index1) in data[qIndex].items" v-if="index1 === 7" @click="checkEvent(index1)"
                            :class="[{active: activeIndex === index1}, {color1: index1 === rightIndex}, {color2: index1 === errorIndex}]">
                            <span :class="[{sign1: index1 === rightIndex}, {sign2: index1 === errorIndex}, 'sign']">H</span>
                            {{a}}
                        </li>
                    </ul>
                </div>
        ...
        <div class="btn-box">
            ...
            <div class="right">
                <div :class="[{button: qIndex !== data.length - 1}, {button1: qIndex === data.length - 1}]"
                     @click="nextEvent">
                    下一题
                </div>
            </div>
        ...
        <div class="footer">
            ...
            <div class="btn" @click="errorListEvent">错题本<span
                    v-if="errorHtmlData.length">({{errorHtmlData.length}})</span></div>
        </div>
            ...
            <div slot="modalbody">
                ...
                <ul>
                    <li v-for="item in allQuestionState">
                        <div class="item" v-if="item.state === 1" @click="allQuestionEvent(item.id)">{{item.id + 1}}
                        </div>
                        <div class="item right" v-if="item.state === 2" @click="allQuestionEvent(item.id)">{{item.id +
                            1}}
                        </div>
                        <div class="item error" v-if="item.state === 3" @click="allQuestionEvent(item.id)">{{item.id +
                            1}}
                        </div>
                    </li>
                </ul>
...
<script>
  ...
  export default {
    ...
    computed: {
      qIndex () {
        return this.$store.state.qIndex;
      },
      thisIndex () {
        return this.$store.getters.thisQuestion.id;
      },
      thisTitle () {
        return this.$store.getters.thisQuestion.title;
      }
    },
    /*
    watch: {
    ...
    methods: {
      checkEvent (num) {
        if (!this.questionList[this.$store.state.qIndex]) { // 判断当前题目是否做过
          ...
          if (this.data[this.$store.state.qIndex].answer === num) { // 做对了
            this.rightIndex = num;
            this.questionList[this.$store.state.qIndex] = {userAns: num, rightAns: num};
            this.allQuestionState[this.$store.state.qIndex].state = 2;
          } else { // 做错了
            this.errorIndex = num;
            this.rightIndex = this.data[this.$store.state.qIndex].answer;
            this.questionList[this.$store.state.qIndex] = {userAns: num, rightAns: this.data[this.$store.state.qIndex].answer};
            this.errorHtmlData.push(this.$store.state.qIndex + 1);
            this.allQuestionState[this.$store.state.qIndex].state = 3;
          }
        }
      },
      preEvent () {
        if (this.$store.state.qIndex !== 0) {
          // this.qIndex -= 1;
          this.$store.commit('preEvent');
          this.activeIndex = -1;
          this.rightIndex = -1;
          this.errorIndex = -1;
          this.ansState = false;
      ...
      qIndexChange () {
        ...
        this.rightIndex = -1;
        this.errorIndex = -1;
        if (this.questionList[this.$store.state.qIndex]) { // 已答列表是否存在
          if (this.questionList[this.$store.state.qIndex].rightAns === this.questionList[this.$store.state.qIndex].userAns) { // 答案是否正确
            this.rightIndex = this.questionList[this.$store.state.qIndex].rightAns;
          } else {
            this.rightIndex = this.questionList[this.$store.state.qIndex].rightAns;
            this.errorIndex = this.questionList[this.$store.state.qIndex].userAns;
          }
        }
      ...
      answerEvent () {
        if (this.questionList[this.$store.state.qIndex]) this.ansState = !this.ansState;
        else this.showToast('未做题目不能看答案');
      },
      collectionEvent () {
        if (this.collectionState) {
          this.collectionList[this.$store.state.qIndex] = false;
          this.collectionState = !this.collectionState;
          this.collectionText = '收藏';
        } else {
          this.collectionList[this.$store.state.qIndex] = true;
          this.collectionState = !this.collectionState;
          this.collectionText = '取消收藏';
        }
      ...
      errInEvent (num) {
        ...
        this.$store.commit('errInEvent', num);
      },
      collectInEvent (num) {
        this.$refs.collectList.modelClose();
        this.$store.commit('collectInEvent', num);
      },
      allQuestionEvent (num) {
        this.$refs.list.modelClose();
        this.$store.commit('allQuestionEvent', num);
      }
    }
  };
</script>
